...
import com.cdac.kiosk.api.HttpUtil;
...
import com.cdac.kiosk.exception.AuthException;
...
public class OnboardNetworkConfigController extends AbstractBaseController {
    ...
    public void initialize() {
        ...
        kioskUrlTextField.setOnKeyPressed(event -> clearMessageLabelIfNotBlank());
    }
    ...
    private void continueBtnAction() {
        ...
        String[] kioskUrlParts = kioskUrlTextField.getText().split(":");
        if (kioskUrlParts.length < 2) {
            messageLabel.setText("Invalid KIOSK URL.");
            return;
        }
        if (!"http".equalsIgnoreCase(kioskUrlParts[0]) && !"https".equalsIgnoreCase(kioskUrlParts[0])) {
            messageLabel.setText("Invalid KIOSK URL scheme. Use either 'http' or 'https'.");
            return;
        }
        kioskUrlTextField.setText(kioskUrlTextField.getText().trim().toLowerCase()); // in case if user type in uppercase

        // should ask for the confirmation before saving
        disableControls(backBtn, continueBtn, hostnameTextField, ipAddressTextField, subnetMaskTextField, defaultGatewayTextField, dnsIpTextField, ldapUrlTextField, mafisUrlTextField, kioskUrlTextField);
    ...
    private void saveChanges() {
        try {
            ...
            PropertyFile.changePropertyValue(PropertyName.KIOSK_URL, kioskUrlTextField.getText().trim());
//            // only do for production as there is no ldap connection in MISCOS
            ...
            try {
                MafisServerApi.fetchOnboardingDetails(new OnboardingReqDto("test", "3"));
            } catch (Exception ex) {
            ...
            // test connection with the kiosk server: only proceed if connection is established
            try {
                HttpUtil.sendHttpRequest(HttpUtil.createGetHttpRequest(kioskUrlTextField.getText().trim()));
            } catch (Exception ex) {
                if (ex.getMessage().toLowerCase().contains("invalid url or ip") || ex.getMessage().toLowerCase().contains("connection timeout")) {
                    throw new GenericException("Invalid KIOSK URL or connection timeout.");
                }
            }
            ...
            enableControls(backBtn, continueBtn, hostnameTextField, ipAddressTextField, subnetMaskTextField, defaultGatewayTextField, dnsIpTextField, ldapUrlTextField, mafisUrlTextField, kioskUrlTextField);
            if (ex instanceof InterruptedException) {
                Thread.currentThread().interrupt();
            }
        }
